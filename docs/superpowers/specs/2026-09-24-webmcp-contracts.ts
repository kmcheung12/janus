/** Normative v1 design contract; not wired into application code. */
...
/** Browser-only ownership; neither object is serialized to MCP or WebSocket. */
export interface BrowserLiveInstance {
  pageId: PageId;
  documentId: Id;
  descriptor: ToolDescriptor;
  // Only Janus-owned registrations have a controller. Never unregister a site's tools.
...
export type ExtensionToDaemon = Hello | PagesSync | ToolsChanged | ToolResult | PageRemoved | Heartbeat
  | DraftUpsert | DraftRemoved | DefinitionResult;
export type DaemonToExtension = HelloAck | ExecuteTool | CancelTool | Heartbeat | DefinitionProposed;
